...
losing anything.** Current as of 24 Aug 2026.

---

## Update, 24 August 2026

Everything below was written on 18 August and still holds, with the additions
in this block on top of it. `CHANGELOG.md` is the same story told once, in
order, for somebody arriving at the repository cold.

**Shipped since 18 August.**

- **Learn / Quiz / Improve / Test.** The app is four sections now rather than
  one quiz screen, each handing off to the next. One migration,
  `supabase/migrations/astro_sections.sql`: five tables, two nullable columns,
  25 functions, and it touches nothing that already existed.
- **219 lessons**, one per subtopic across all six courses, keyed to the same
  misconception tags the questions carry. Zero subtopics are without one.
- **316 lesson diagrams**, 158 authored as SVG and rendered to PNG in both
  themes ahead of time by `tools/render_diagrams.py`, so the app needs no SVG
  package and reuses the `Image.network` path the question figures use. With
  the 60 question figures that is 376 PNGs under `web/figures/`.
- **Dark mode**, which the 18 August version of this file lists under
  "discussed, not chosen". The palette is one swappable object behind the
  existing colour names, so `kInk` and the rest did not move; the cost was the
  `const` on the widgets that held them. Stored per profile as `theme_pref`.
- **Branding**, generated by `tools/make_icons.py`, and the privacy policy and
  terms, authored as Markdown in `docs/` and rendered to `web/` by
  `tools/render_legal.py`.
- **Two more test suites**, `tests/test_sections.sql` (66 checks) and
  `tests/test_rpc_names.sql` (12).
- **The repository is under version control**, and the derived question bundles
  now sit in `supabase/migrations/bundles/` with the retired migrations in
  `_superseded/`. Nothing was deleted.

**The live database has moved on too.** Checked on 24 August: 1,600 questions,
60 figures, **219 lessons**, 92 functions, 8 accounts, 316 attempts, and two
practice tests taken. So the Grade 9, 11 and 12 banks, `astro_sections.sql` and
all six lesson files are applied, which the Outstanding list below predates.
`docs/SQL_ORDER.md` route A predates it as well.

`lib/main.dart` is 15,570 lines.
...
| Thing | Where |
|---|---|
| App | Flutter Web, one file: `lib/main.dart` (~15,570 lines). **Keep it one file** — deliberate, for a solo beginner maintainer. Do not split. |
...
Three more suites since 18 August:

| Suite | Checks | Covers |
|---|---|---|
| `tests/test_sections.sql` | 66 | Learn, Improve, Test, percentages, enrolment. Run it on a different scratch database from `test_ama.sql`, or before it: both create their own fixture accounts and neither clears the other's. |
| `tests/test_rpc_names.sql` | 12 | Calls every RPC the way PostgREST does, by named argument. A renamed SQL parameter is a runtime failure that `flutter analyze` cannot see, and nothing else catches it. |
| `test/widget_test.dart` | 14 | The percentage rule, in the browser. `flutter test`. |

All four were re-run on 24 August 2026 against a scratch Postgres built from
the repository alone, by route B of `docs/SQL_ORDER.md`. 212, 66, 12 and 14,
all passing, `flutter analyze` clean. The scratch database came out at 1,600
...
- ~~Load the Grade 9, 11 and 12 banks~~ — **done.** All six courses are live.
- ~~Run `avatars.sql`~~ — **done**, and it is folded into the setup file now.
- Copy `web/figures/` into the deploy so all 376 images ship with the app
- Deploy the August build. The hosted site still serves an older one, and that
  is the next step rather than more features.

**Mine:**
- The bank is authored, audited and repaired. All 40 units are on disk and
  **all 40 pass the full gate**, MPM2D included since its option lists were
  rotated.
- Of the three things found and deliberately not fixed, two are closed. The
  **longest-option tell** is gone: 398 of 1600, 24.9% against a chance 25%.
  **MPM2D Quadratics check 12** passes, at the accepted cost to historical
  `attempts.chosen_index` recorded above. The **notation split** between
  Unicode and ASCII files is still open.
...
**Discussed, not chosen:** tutor booking / "Uber for tutors" (a project in
itself, not a feature). Dark mode was on this list on 18 August and shipped on
the 23rd. The five SVG logo directions in `brand/` were declined; the mark that
shipped came out of `tools/make_icons.py`.
